...
proof_system::honk::sumcheck::SumcheckProverRound "Sumcheck Round Prover".

 ## Input and Parameters
 The following constants are used:
 - \f$ d \f$ \ref multivariate_d "the number of variables"
 - \f$ n \f$ \ref multivariate_n "the size of the hypercube", i.e. \f$ 2^d\f$.
 - \f$ D = \f$  \ref proof_system::honk::sumcheck::SumcheckProverRound< Flavor >::BATCHED_RELATION_PARTIAL_LENGTH "the
maximum partial degree of" \f$\tilde{F}\f$.
...
P_N(u_0,\ldots, u_i, \vec \ell)\f$ for \f$\vec \ell \in \{0,1\}^{d-1-i}\f$. The details are specified in \ref
partially_evaluate "the corresponding docs."
 ...
 * For a random challenge \f$\zeta \in \mathbb{F}^{\times}\f$, by \ref barretenberg::PowUnivariate "Definition of
`pow`", we have \f{align}{ \sum_{\vec \ell \in \{0,1\}^d} \tilde{F}(\vec \ell) = \sum_{\vec \ell \in \{0,1\}^d} pow(\vec
\ell) \cdot F(\vec \ell) = \sum_{\vec \ell \in \{0,1\}^d} \zeta^{\ell}\cdot P(\vec \ell) \f}
 * Hence if Sumcheck passes, then with high-probability \f$ F(\vec \ell) = 0\f$ for all \f$\vec \ell\f$.
 *

 *
 * In Round \f$i\f$, a univariate polynomial \f$ \tilde S^{i}(X_{i}) \f$ for the relation defined by \f$ \tilde{F}
(X)\f$ is computed as follows. First, we introduce notation
 - \f$ \zeta_k = \zeta^{2^k} \f$
...
\ell) \right) \f$
 ...
 As explained in \ref barretenberg::PowUnivariate "Definition of `pow`",
 \f{align}{
...
method \ref proof_system::honk::sumcheck::SumcheckProverRound::compute_univariate "compute_univariate". The
implementation consists of the following sub-methods:

 - \ref proof_system::honk::sumcheck::SumcheckProverRound::extend_edges "Extend evaluations" of linear univariate
polynomials \f$ P_j(u_0,\ldots, u_{i-1}, X_i, \vec \ell) \f$ to the domain \f$0,\ldots, D\f$.
 - \ref proof_system::honk::sumcheck::SumcheckProverRound::accumulate_relation_univariates "Accumulate per-relation
contributions" of the extended polynomials to \f$ T^i(X_i)\f$
 - \ref proof_system::honk::sumcheck::SumcheckProverRound::extend_and_batch_univariates "Extend and batch the
per-relation contibutions" multiplying by the evaluations of \f$ ( (1−X_i) + X_i\cdot \zeta_i ) \f$ and the batching
challenge \f$\alpha \f$.
...
transcript. These operations are taken care of by \ref proof_system::honk::BaseTranscript "Transcript Class" methods.
## Output
The Sumcheck output is specified by \ref proof_system::honk::sumcheck::SumcheckOutput< Flavor >.
 */
template <typename Flavor> class SumcheckProver {
  ...
  public:
    ...
    * @brief Container for partially evaluated Prover Polynomials at a current challenge. Upon computing challenge \f$
    u_i \f$, the first \f$2^{d-1-i}\f$ rows are updated using \ref proof_system::honk::sumcheck::SumcheckProver< Flavor
    >::partially_evaluate "partially evaluate" method.
    ...
    */
     ...
     *
     * @details See Detailed description of \ref proof_system::honk::sumcheck::SumcheckProver< Flavor > "Sumcheck Prover
     * <Flavor>
     ...
     * Specialization for array, see \ref proof_system::honk::sumcheck::SumcheckProver< Flavor >::partially_evaluate	*
     * "generic version".
     */
 ...
 * - Extract Round Univariate's \f$\tilde{F}\f$ evaluations at \f$0,\ldots, D \f$ from the transcript using \ref
 proof_system::honk::BaseTranscript::receive_from_prover "receive_from_prover" method from BaseTranscript Flavor
 * - \ref proof_system::honk::sumcheck::SumcheckVerifierRound< Flavor >::check_sum "Check target sum": \f$\quad \sigma_{
 i } \stackrel{?}{=}  \tilde{S}^i(0) + \tilde{S}^i(1)  \f$
 * - Compute the challenge \f$u_i\f$ from the transcript using \ref proof_system::honk::BaseTranscript::get_challenge
 "get_challenge" method from BaseTranscript Flavor.
 * - \ref proof_system::honk::sumcheck::SumcheckVerifierRound< Flavor >::compute_next_target_sum "Compute next target
 sum" :\f$ \quad \sigma_{i+1} \gets \tilde{S}^i(u_i) \f$
 ...
 * - Extract \ref ClaimedEvaluations of prover polynomials \f$P_1,\ldots, P_N\f$ at the challenge point \f$
 (u_0,\ldots,u_{d-1}) \f$ from the transcript and \ref proof_system::honk::sumcheck::SumcheckVerifierRound< Flavor
...
class SumcheckVerifier {
  ...
  public:
    ...
    const size_t multivariate_d;
    /**
     * @brief Declare an instance of \ref SumcheckVerifierRound< Flavor >
     *
     */
    ...
    {
        bool verified(true);
        // Compute challenge for the \ref barretenberg::PowUnivariate "Pow Univariate"
        FF zeta = transcript->get_challenge("Sumcheck:zeta");
