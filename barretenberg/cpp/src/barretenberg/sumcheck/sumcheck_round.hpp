...
template <typename Flavor> class SumcheckProverRound {
  ...
  public:
     ...
     * In both cases, in Round \f$ i \f$, \ref extend_edges "the method" receives \f$(0, \vec \ell) \in
     ...
     input in the first round, or from the #multivariates table. Using general method \ref
     barretenberg::Univariate::extend_to "extend_to", the evaluations of these polynomials are being extended from the
     ...
     * This method invokes more general \ref barretenberg::Univariate::extend_to "extend_to" method that in this case
        ...
        const FF alpha)
    {
        ...
        parallel_for(num_threads, [&](size_t thread_idx) {
            ...
            for (size_t edge_idx = start; edge_idx < end; edge_idx += 2) {
                ...
                // `barretenberg::PowUnivariate`, scale it by the pow polynomial's constant and zeta power "c_l ⋅
    ...
    /**
     * @brief Given a tuple of tuples of extended per-relation contributions,  \f$ (t_0, t_1, ...,
     ...
     * applying \ref barretenberg::RelationUtils::add_nested_tuples "add_nested_tuples" method to join the threads.
     *
     ...
     * @param pow_univariate Round Pow-factor  \f$ ( (1−X_i) + X_i\cdot \zeta_i )\f$.
     */
  ...
  private:
     ...
     * proof_system::honk::sumcheck::SumcheckProver< Flavor > "Sumcheck Prover detailed description" is computed  as
     * follows:
